...
(
  ...
  try docker cp "${SERVER_PUBLIC_CHAIN_CERT_PATH}" "${VAULT_INIT_KEYS_CONTAINER_NAME}:/vault/init-keys/ca.crt"
  ...
  try docker exec ${VAULT_INIT_KEYS_CONTAINER_NAME} chown vault:vault /vault/init-keys/server.crt /vault/init-keys/server.key /vault/init-keys/ca.crt
  ...
  try docker exec ${VAULT_INIT_KEYS_CONTAINER_NAME} chmod 644 /vault/init-keys/ca.crt
